            ...
            for current_x in filter(lambda elem: elem is not None, (x,)): # (x_pure_rand, x, x_e4m3)):
                for with_topk in (use_topk,): # (False, True):
                    ...
                    if not with_topk:
                        ...
                        if previous_mode:
                            dispatch_args.update({'previous_event': buffer.capture()})
                        recv_cache_x, _, _, _, _, event = buffer.dispatch(**dispatch_args)
                        event.current_stream_wait() if async_mode else ()
                        recv_cache_x = per_token_cast_back(*recv_cache_x) if isinstance(recv_cache_x, tuple) else recv_cache_x
                        if current_x is not x_pure_rand:
                    ...
                    # NOTE: the send_head will be modified in-place in intranode::cached_notify_combine for the entries == -1 to the position p of next valid token (encoded to -p-1)
                    # since the combine kernel needs to know the channel position when iterating at this token, even though it is not sent to the target rank
