...
void testDispatch(
    ...
    // where indices_per_rank_h[r][i * expertsPerToken + j] indicates the jth expert index of local token i to send to for rank r
    ...
    // then indices_per_rank_h[0][0] = 7, indices_per_rank_h[0][1] = 4, indices_per_rank_h[0][6] = 0, indices_per_rank_h[0][7] = 5
    std::vector<std::vector<uint32_t>> indices_per_rank_h(world_size, std::vector<uint32_t>(localTokens * expertsPerToken, 0));
    for (int r = 0; r < world_size; ++r) {
        for (int i = 0; i < localTokens; ++i) {
            // For each token, assign it to other rank
            for (int j = 0; j < expertsPerToken; ++j) {
                // mixed assign
                indices_per_rank_h[r][i * expertsPerToken + j] = (r * 17 + i * 11 + j * 13 + 23) % numExperts;
            }
        }   
    }
    ...
    print_transfer_information(
        tokens_h, 
        const_cast<const std::vector<std::vector<uint32_t>> &>(indices_per_rank_h), 
        numLocalExperts,
        localTokens, 
        hiddenDim, 
        expertsPerToken, 
    ...
    DeviceBuffer<uint32_t> indices_d(indices_per_rank_h[rank]);
